...
class CurrencyFetcher:
    ...
    @staticmethod
    def get_rate(moeda: str = "USD") -> Optional[float]:
        """
        Obtém apenas a taxa de conversão de uma moeda para BRL.

        Args:
            moeda: Código da moeda (ex: USD, EUR, GBP)

        Returns:
            Taxa de conversão para BRL ou None se erro
        """
        resultado = CurrencyFetcher.get_exchange_rate(from_currency=moeda, to_currency="BRL")

        if resultado and "rate" in resultado:
